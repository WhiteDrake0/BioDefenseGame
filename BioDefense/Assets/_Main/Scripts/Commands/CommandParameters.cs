...
namespace COMMAND
{
    ...
    {
        private const char PARAMETER_INDENTIFIER = '-';
        ...
        public CommandParameters(string[] parameterArray)
        {
            ...
            {
                if (parameterArray[i].StartsWith(PARAMETER_INDENTIFIER) && !float.TryParse(parameterArray[i], out _))
                {
                    string pName = parameterArray[i];
                    string pValue = "";

                    if (i + 1 < parameterArray.Length && !parameterArray[i + 1].StartsWith(PARAMETER_INDENTIFIER))
                    {
                    ...
                    unlabelParameters.Add(parameterArray[i]);
